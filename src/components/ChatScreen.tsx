    ...
    <div className={`flex h-screen overflow-hidden font-sans ${isDarkMode ? 'bg-bg-dark text-white' : 'bg-bg-light text-slate-900'}`}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-2 space-y-1">
          <button className={`w-full flex items-center gap-3 p-3 text-sm rounded-xl transition-all ${isDarkMode ? 'hover:bg-white/5 text-white/80' : 'hover:bg-black/5 text-slate-600'}`}>
            <MessageSquare size={18} />
            <span>Chat History</span>
          </button>
          <div className={`mt-8 px-2 py-4 text-[10px] font-semibold uppercase tracking-widest text-center ${isDarkMode ? 'text-white/30' : 'text-slate-400'}`}>
            Recent Activity
          </div>
          <div className="px-3 py-8 text-center space-y-2">
            <HelpCircle size={32} className={`mx-auto ${isDarkMode ? 'text-white/10' : 'text-black/5'}`} />
            <p className={`text-xs ${isDarkMode ? 'text-white/30' : 'text-slate-400'}`}>Your recent chats will appear here</p>
          </div>
        </div>
